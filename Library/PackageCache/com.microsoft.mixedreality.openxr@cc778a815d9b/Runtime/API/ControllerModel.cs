...
namespace Microsoft.MixedReality.OpenXR
{
    ...
    public class ControllerModel
    {
        ...
        public bool TryGetControllerModelKey(out ulong modelKey)
        {
            ...
            {
                Debug.LogWarning($"TryGetControllerModelKey: The controller model is not supported; missing OpenXR extension {ControllerModelExtension}");
                modelKey = 0;
        ...
        public Task<byte[]> TryGetControllerModel(ulong modelKey)
        {
            if (m_token == NativeLibToken.Invalid || !m_controllerModelExtensionEnabled)
            {
                Debug.LogWarning($"TryGetControllerModel: The controller model is not supported; missing OpenXR extension {ControllerModelExtension}");
            ...
            Task<byte[]> newTask = Task.Run(() =>
            {
                if (NativeLib.TryGetControllerModel(m_token, modelKey, 0, out uint bufferCapacity))
                {
                    ...
                    if (NativeLib.TryGetControllerModel(m_token, modelKey, bufferCapacity, out _, modelBuffer))
                    {
                        return modelBuffer;
                    }
                }
